        ...
        for (Class i : c.getInterfaces())
            if (DBusInterface.class.equals(i)) {
                ...
                if (null != c.getAnnotation(DBusInterfaceName.class)) {
                    String name = ((DBusInterfaceName) c.getAnnotation(DBusInterfaceName.class))
                            .value();
                    ...
                    if (Modifier.isPublic(meth.getModifiers())) {
                            ...
                            if (DBusExecutionException.class.isAssignableFrom(ex))
                                introspectiondata +=
                                        "   <annotation name=\"org.freedesktop.DBus.Method.Error\" value=\""
                                                + AbstractConnection.dollar_pattern.matcher(
                        ...
                        if (!Void.TYPE.equals(meth.getGenericReturnType())) {
                            if (Tuple.class.isAssignableFrom((Class) meth.getReturnType())) {
                                ...
                                throw new DBusException(
                                        _("Return type of Object[] cannot be introspected properly"));
                            } else
                                for (String s : Marshalling
                                        .getDBusType(meth.getGenericReturnType()))
                                    introspectiondata += "   <arg type=\"" + s
                                            + "\" direction=\"out\"/>\n";
                        }
                ...
                for (Class sig : c.getDeclaredClasses())
                    if (DBusSignal.class.isAssignableFrom(sig)) {
                        ...
                        if (sig.isAnnotationPresent(DBusMemberName.class)) {
                            name = ((DBusMemberName) sig.getAnnotation(DBusMemberName.class))
                                    .value();
