...
{
    private static final String TAG="DBus-Marshal";
            ...
            throws DBusException
    {
        ...
        if (c instanceof TypeVariable)
            ...
            String[] s = recursiveGetDBusType(((GenericArrayType) c).getGenericComponentType(),
                    false, level + 1);
            if (s.length != 1)
                throw new DBusException(_("Multi-valued array types not permitted"));
            out[level].append(s[0]);
        } else if ((c instanceof Class &&
                DBusSerializable.class.isAssignableFrom((Class<? extends Object>) c))
                ||
                (c instanceof ParameterizedType &&
                DBusSerializable.class
                        .isAssignableFrom((Class<? extends Object>) ((ParameterizedType) c)
                                .getRawType()))) {
            ...
            if (c instanceof Class) {
                ...
                for (Method m : ((Class<? extends Object>) ((ParameterizedType) c).getRawType())
                        .getDeclaredMethods())
                    if (m.getName().equals("deserialize"))
                        newtypes = m.getGenericParameterTypes();
            ...
            if (null == newtypes)
                throw new DBusException(
                        _("Serializable classes must implement a deserialize method"));
            ...
            for (int j = 0; j < sigs.length; j++) {
                ...
                if (1 != ss.length)
                    throw new DBusException(
                            _("Serializable classes must serialize to native DBus types"));
                sigs[j] = ss[0];
            }
            return sigs;
        }
            ...
            if (Type.class.equals(((Class<? extends Object>) c).getComponentType()))
                ...
                out[level].append((char) Message.ArgumentType.ARRAY);
                String[] s = recursiveGetDBusType(((Class<? extends Object>) c).getComponentType(),
                        false, level + 1);
                if (s.length != 1)
                    throw new DBusException(_("Multi-valued array types not permitted"));
                out[level].append(s[0]);
        ...
        debug(VERBOSE, "Converted Java type: " + c + " to D-Bus Type: " + out[level]);
                ...
                switch (dbus.charAt(i)) {
                    ...
                    default:
                        throw new DBusException(MessageFormat.format(
                                _("Failed to parse DBus type signature: {0} ({1})."), new Object[] {
                                        dbus, dbus.charAt(i)
                                }));
                }
            return i;
            ...
            AbstractConnection conn) throws DBusException
    {
        ...
        for (int i = 0; i < parameters.length; i++) {
            debug(VERBOSE, "Converting " + i + " from " + parameters[i] + " to "
                            + types[i]);
            ...
            if (parameters[i] instanceof DBusSerializable) {
                for (Method m : parameters[i].getClass().getDeclaredMethods())
                    if (m.getName().equals("deserialize")) {
                        ...
                        System.arraycopy(types, i + 1, expand, i + newtypes.length, types.length
                                - i - 1);
                        types = expand;
                        ...
                        System.arraycopy(parameters, i + 1, exparams, i + newparams.length,
                ...
                System.arraycopy(parameters, i + 1, exparams, i + newparams.length,
                        parameters.length - i - 1);
                parameters = exparams;
                debug(VERBOSE, "New params: " + Arrays.deepToString(parameters)
                            + " new types: " + Arrays.deepToString(types));
                i--;
    ...
    static Object deSerializeParameter(Object parameter, Type type, AbstractConnection conn)
            throws Exception
    {
        debug(VERBOSE, "Deserializing from " + parameter.getClass() + " to " + 
                type.getClass());
        if (null == parameter)
            return null;
        ...
        if (parameter instanceof Object[] &&
            ...
            debug(VERBOSE, "Creating Struct " + type + " from " + parameter);
        ...
        if (parameter instanceof DBusMap) {
            debug(VERBOSE, "Deserializing a Map");
    ...
    static List<Object> deSerializeParameters(List<Object> parameters, Type type,
            AbstractConnection conn) throws Exception
    {
        debug(VERBOSE, "Deserializing from " + parameters + " to " + type);
        if (null == parameters)
            return null;
        for (int i = 0; i < parameters.size(); i++) {
             ...
             * Debug.print(Debug.ERR, AIOOBe); throw new DBusException(
             * "Not enough elements to create custom object from serialized data ("
    ...
    @SuppressWarnings("unchecked")
    static Object[] deSerializeParameters(Object[] parameters, Type[] types, AbstractConnection conn)
            throws Exception
    {
        debug(VERBOSE, "Deserializing from " + Arrays.deepToString(parameters)
        ...
        if (types.length == 1
                ...
                && Tuple.class
                        .isAssignableFrom((Class) ((ParameterizedType) types[0]).getRawType())) {
        ...
        for (int i = 0; i < parameters.length; i++) {
            ...
            if ((types[i] instanceof Class &&
                    ...
                    DBusSerializable.class
                            .isAssignableFrom((Class<? extends Object>) ((ParameterizedType) types[i])
                                    .getRawType()))) {
                ...
                for (Method m : dsc.getDeclaredMethods())
                    if (m.getName().equals("deserialize")) {
                        ...
                        try {
                            ...
                            throw new DBusException(
                                    MessageFormat
                                            .format(_("Not enough elements to create custom object from serialized data ({0} < {1})."),
                                                    new Object[] {
                                                            parameters.length - i, newtypes.length
                                                    }));
                        }
