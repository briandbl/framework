...
public class Transport
{
        ...
        public int do_challenge(int auth, Command c) throws IOException
        {
            switch (auth) {
                case AUTH_SHA:
                    ...
                    if (null == cookie) {
                        if (Debug.debug)
                            Debug.print(Debug.DEBUG, "Did not find a cookie in context " + context
                                    + " with ID " + ID);
                        return ERROR;
                    }
        ...
        public int do_response(int auth, String Uid, String kernelUid, Command c)
        {
            ...
            switch (auth) {
                case AUTH_NONE:
                    switch (c.getMechs()) {
                        ...
                        case AUTH_SHA:
                            ...
                            if (Debug.debug)
                                Debug.print(Debug.DEBUG, "Sending challenge: " + context + ' ' + id
                                        + ' ' + challenge);
                ...
                case AUTH_SHA:
                    ...
                    if (Debug.debug)
                        Debug.print(Debug.DEBUG, "Authenticating Hash; data=" + prehash
    ...
    public void connect(LocalSocketAddress address, int timeout) throws IOException
    {
        ...
        // if (!(new SASL()).auth(mode, types, address.getParameter("guid"),
        // out, in, us)) {
        // out.close();
